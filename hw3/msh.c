...
void append_to_arglist( char* arg, struct stringlist* arglist );
...
int main()
{
    ...
    cmdlist.list = malloc( 1 * sizeof(struct stringlist) );
    ...
    arglist.list = malloc( 1 * sizeof(struct string) );
    ...
    while( 1 )
    {
        ...
        if( strcmp(arglist.list[0].str, "exit") == 0 )
        {
            int i;
        ...
        if( strcmp(arglist.list[0].str, "cd") == 0 )
        {
            int return_code = chdir( arglist.list[1].str );
            if( return_code == -1 )
...
void append_to_arglist( char* arg, struct stringlist* arglist )
{
    ...
    (*arglist).list = realloc( (*arglist).list, ((*arglist).len + 1) * sizeof(struct string) );
    if( (*arglist).list == NULL )
    {
        perror( "Unable to allocate memory for argument list" );
        exit( 1 );
